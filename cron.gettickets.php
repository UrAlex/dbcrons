#!/usr/bin/php
<?php	
	include_once("../dblibraries/lib.tickets.php");
	...
	$logmsg = "Tikects from $startdate to $enddate\n\n";
		...
		if($apikey && $venueurl){	
		    ...
		    if($numtickets > 0){
			    for($a=0; $a<count($tickets); $a++){
			    	...
			    	$result_cust = Customers::add_customers($customer, 28, $venueid, "", "", $showid);		    
			    	...
			    	$result_tick = Tickets::add_ticket($ticketid, $showid, $cid, '', $ticketinfo);
			    	$newticketid = $result_tick['ticketid'];
			    	$logtickets .= "External TicketID: $ticketid\nNew TicketID: $newticketid\nTicket Info:\n"; 
			    	$logtickets .= "   firstname : $cfirstname\n   lastname : $clastname\n   email : $cemail\n   cellphone : $cphone\n   dob : $cbirthday\n   zipcode : $czip\n   over21 : $cover21\n   gender : $cgender\n   checkedin : $tcheckedin\n   quantity : $tquantity\n   revenue : $trevenue\n   servicefee : $tservicefee\n   tax : $ttax\n   total : $ttotal\n   discount : $tdiscount\n   timetstamp : $timetstamp\n\n";			    	
				}
